...
namespace xarm_control
{
	...
	std::string XArmHW::_controller_error_interpreter(int err)
	{
		switch(err)
		{
			case 1:
	...
	bool XArmHW::_xarm_is_ready_read(void)
    {
        ...
        if (curr_err != 0) {
            if (last_err != curr_err) {
                ROS_ERROR("[ns: %s] xArm Controller Error detected! Code: C%d [ %s ]", hw_ns_.c_str(), curr_err, _controller_error_interpreter(curr_err).c_str());
            }
        }
        last_err = curr_err;
        return last_err == 0;
